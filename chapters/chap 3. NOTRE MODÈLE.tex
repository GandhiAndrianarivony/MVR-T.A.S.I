...
\subsubsection{Décomposition Duale.}
...
\subsubsection{Lagrangiens Augmentés et la Méthode des Multiplicateurs.}
...
% ===================================================================
\subsection{Transformées en Ondelettes Biorthogonales}
...
Cette thèse se propose d'explorer deux familles majeures d'ondelettes : les ondelettes orthogonales et les ondelettes biorthogonales. La distinction entre ces deux approches, bien que subtile dans sa formulation mathématique, a des implications profondes sur leurs propriétés et leurs domaines d'application privilégiés.
...
Avant de plonger au cœur de notre étude comparative, il est essentiel de poser les bases conceptuelles et mathématiques qui sous-tendent la théorie des ondelettes. Ce introductif vise à familiariser le lecteur avec les concepts fondamentaux, depuis les prémices de l'analyse fréquentielle jusqu'à la formulation moderne de la transformation en ondelettes.

\subsubsection{De Fourier aux Ondelettes : Une Évolution Nécessaire}

L'orthogonalité est un aspect important lié au traitement du signal car c'est une méthode facile dans les langages pour construire des structures de contrôle et de données. Sa nature est simple, elle a donc de nombreuses conceptions orthogonales avec moins d'attentes. Grâce à cela, nous sommes capables de lire, d'écrire et d'apprendre n'importe quel aspect d'un langage de programmation. Les significations des caractéristiques de l'orthogonalité sont indépendantes pour chaque contexte [1], [2], [3]. L'orthogonalité reflète les angles droits, c'est-à-dire \(90^{\circ}\).

\subsubsection{ Concepts Fondamentaux}

\subsubsection{ Orthogonalité}

Dans le modèle mathématique, l'orthogonalité décrit la perpendicularité pour l'algèbre linéaire avec des formes bilinéaires. Il y a deux éléments, x et y, qui sont orthogonaux. Si B(x, y) = angle de 90 degrés. L'espace vectoriel a des vecteurs non nuls, qui sont auto-orthogonaux. La famille de fonctions orthogonales est utile dans la formation de la base.

<center>Figure 1.1. Orthogonalité </center>

\subsubsection{Signal Orthogonal}

Le signal orthogonal est formé à l'aide du filtre miroir conjugué. Le filtre miroir divise l'axe des fréquences en différents intervalles. Ces intervalles ont des tailles différentes. Le banc de filtres conjugués correspond à diverses bases de paquets. Il est nécessaire d'isoler les petites fenêtres, c'est-à-dire la fenêtre d'ondelettes en présence du système variant dans le temps, car elle est responsable de la modification des propriétés du signal. Généralement, les bases cosinus locales sont obtenues par la simple multiplication des fenêtres et de la fonction cosinus. Ici, un paquet d'ondelettes divise l'axe des fréquences et est systématiquement transcrit dans le temps. Les bases cosinus locales divisent l'axe du temps et sont systématiquement transcrites en fréquence.
...
\[\int_{-\infty}^{\infty}\theta (x)\theta (x - k)dx = \delta_{0,k} \quad (1)\]

Cette condition donne la condition équivalente pour l'orthogonalité.

\subsubsection{Transformée de Fourier}

La transformée de Fourier est mathématiquement représentée par :

\[F(\omega) = \int_{-\infty}^{\infty}f(t)\exp (-j\omega t)dt \quad (2)\]

La conversion du signal du domaine temporel au domaine fréquentiel se fait à l'aide de la transformée de Fourier en considérant l'axe temporel complet.

En fait, les transformées de Fourier indiquent des informations sur le signal. Elles montrent les détails concernant la composante de fréquence qui apparaît ou est présente dans le signal. Elles ne donnent pas de détails sur la valeur exacte de la fréquence présente à cet instant précis.

\subsubsection{Transformée de Fourier à Court Terme (STFT)}
...
\subsubsection{Transformation en Ondelettes}

Une ondelette est l'une des fonctions mathématiques utiles pour diviser un signal temporel continu ou une fonction en différentes composantes d'échelle. Fondamentalement, les ondelettes sont des oscillations qui ont une certaine valeur d'amplitude. Elles commencent à zéro, puis augmentent, et diminuent à nouveau pour revenir à 0. Cela ressemble aux brèves oscillations qui se produisent sur un sismographe.
...
Fondamentalement, la transformée en ondelettes entre en jeu au moment où le signal est analysé, à la fois dans les domaines temporel et fréquentiel. Elle est surtout applicable au système variant dans le temps car elle offre une résolution de fréquence variable à la fin du processus de décomposition, en alternant STFT et Fourier. Ici, la STFT supporte la transformée de Fourier pour le temps instantané t avec une fenêtre courte ayant un point médian au temps t. La longueur de la fenêtre est constante pour la STFT, donc elle ne supporte pas l'anatomie multi-résolution des signaux. L'ondelette est différente en ce sens qu'elle fournit à la fois une résolution temporelle et fréquentielle.

La fonction de transformation est appelée ondelette 'Mère'. Le nom d'ondelette est dérivé des bébés ondes. Ces bébés ondes sont appelées une fonction oscillatoire. Le nom 'Mère' entre en jeu car ces ondelettes sont formées à partir de diverses transformations de base.

Considérons l'ondelette mère, qui est une fonction fenêtrée, qui est décalée avec le signal temporel où le temps varie de 0 à T. Les coefficients d'ondelette sont calculés par un processus simple. Après avoir multiplié une partie/morceau du signal dans la fenêtre et l'ondelette mère, leur intégration sur le temps complet aboutit à la génération du coefficient d'ondelette. Techniquement, la fenêtre de l'ondelette mère est aussi appelée la fenêtre de support. En raison de l'apparition d'un problème de résolution lié au temps et à la fréquence, la croyance complète en les ondelettes émane de la transformée de Fourier [6], [7].
...
La fonction ci-dessus est mise à l'échelle par a' puis translatée vers un second scalaire b'. Alors son ondelette mère est représentée par :
...
où :
- \(\mathrm{T} =\) Point quelconque dans le temps
- \(\Psi^{*}(a,T)(t) =\) fonction ondelette
- y(t) \(=\) entrée
...
Le filtre qui satisfait la relation ci-dessus est appelé filtre miroir conjugué [8], [9]. Un filtre miroir conjugué est l'un des moyens les plus simples pour la construction d'une base d'ondelettes orthogonale. Le filtre miroir conjugué et la multi-résolution sont les méthodes de base pour la base d'ondelettes orthogonale [10], [11].

\subsubsection{Propriétés des Ondelettes}
...
\[\begin{array}{l}{d^{1 =}s_{2j} - \frac{1}{2}\left[s_{2j - 1} + s_{2j + 2}\right]}\\ {}\\ {s^{1 =}s_{2j - 1} + \frac{1}{4}\left[d_{j - 1}^{1} + d^{(1)}\right]}\\ {}\\ {D = \frac{1}{\sqrt{2}} d^{(1)}}\\ {}\\ {s_{1} = \sqrt{2} s^{(1)}} \end{array} \quad (10)\]
...
\[\phi_{j,k}(t) = 2^{j / 2}\phi (2^{j}t - k) \quad (16)\]
...
\[\phi (t) = \sqrt{2\sum_{n}h_{n}\phi (2t - n)} \quad (17)\]
...
\[\psi_{j,k}(t) = 2^{j / 2}\psi (2^{j}t - k) \quad (18)\]
...
\[\psi (t) = \sqrt{2\sum_{n}g_{n}\phi (2t - n)} \quad (19)\]
...
\subsection{Ondelettes Biorthogonales : Généralisation et Flexibilité}

\subsubsection{ Introduction}

Dans certains cas, lorsque la condition d'orthogonalité échoue, cela donne lieu à des ondelettes biorthogonales. À ce stade, les ondelettes biorthogonales se forment et l'orthogonalité des ondelettes disparaît.

\subsubsection{ III.2 De l'Orthogonalité à la Biorthogonalité}
...
Une ondelette biorthogonale génère une ondelette et une fonction d'échelle pour la décomposition et une paire supplémentaire pour la reconstruction. Deux fonctions d'échelle sont responsables de la génération de diverses analyses multi-résolution.

\subsubsection{Formulation Mathématique}

Ici, la fonction d'échelle duale est représentée mathématiquement comme suit :

\[\tilde{\phi} (t) = \sqrt{2}\sum_{n}\tilde{h}_{n}\tilde{\phi} (2t - n) \quad (21)\]

De même, l'ondelette duale est représentée par :

\[\tilde{\psi} (t) = \sqrt{2}\sum_{n}\tilde{g}_{n}\tilde{\phi} (2t - n) \quad (22)\]

Maintenant, l'échelle duale est liée comme suit :

\[\begin{array}{l}{g_{n} = (-1)^{n}h_{1 - n}}\\ {\tilde{g}_{n} = (-1)^{n}h_{1 - n}} \end{array} \quad (23)\]
...
La naissance du schéma de lifting vient de Cohen et al. et Swelden. Nous avons supposé que l'ensemble initial \(\{h,\tilde{h}^0,g^0,\tilde{g}\}\) concernait des ondelettes biorthogonales finies. Ainsi, nous avons rassemblé un nouvel ensemble \(\{h,\tilde{h},g,\tilde{g}\}\) , qui consiste en un filtre biorthogonal fini.

\[\begin{array}{l}\tilde{h} (\omega) = \tilde{h}^0 (\omega) + \tilde{g} (\omega)\overline{s(2\omega)} \\ \tilde{g} (\omega) = g^0 (\omega) + h(\omega)s2(\omega) \end{array} \quad (25)\]
...
\[\psi (x) = \psi^0 (x) - \sum_k s_k\phi (x - k) \quad (27)\]

\[\tilde{\phi} (x) = 2\sum_k\tilde{h}_k^0\phi (2x - k) + \sum_k s_{-k}\tilde{\psi} (x - k) \quad (28)\]

\[\tilde{\psi} (x) = 2\sum_k\tilde{g}_k\tilde{\phi} (2x - k) \quad (29)\]

Ici \(s_k\) peut être choisi aléatoirement. La valeur de s est responsable de la manipulation des fonctions duales et des ondelettes dérivées d'une fonction d'échelle simple [15], [16].
...
\[\begin{array}{l}\tilde{V}_j\perp W_j\\ V_j\perp \tilde{W}_j\\ (\tilde{\phi}_{j,l},\phi_{j,l'}) = \delta_{l,l'}\\ (\tilde{\psi}_{j,l},\psi_{j,l'}) = \delta_{j,j'}\delta_{l,l'}\\ (\tilde{\phi}_{j,l},\psi_{j,l'}) = 0\quad et\quad (\tilde{\psi}_{j,l},\phi_{j,l'}) = 0. \end{array} \quad (37)\]
...
Une ondelette biorthogonale est plus avancée qu'une ondelette orthogonale. Cependant, elle nécessite plus d'efforts de calcul et est un processus plus long. Une variation de longueur est la principale différence entre l'ondelette orthogonale et l'ondelette biorthogonale lors de l'analyse, ainsi que dans le processus de synthèse. Dans une ondelette orthogonale, des filtres de même longueur sont utilisés et dans l'ondelette biorthogonale, des filtres de longueurs différentes sont utilisés [4], [5].

La différence entre les ondelettes orthogonales et biorthogonales est qu'une ondelette orthogonale fournit une matrice orthogonale avec transformation unitaire, tandis qu'une ondelette biorthogonale donne une matrice inversible, c'est-à-dire une matrice inverse avec reconstruction parfaite. Dans le cas d'un filtre d'ondelette biorthogonal, le filtre passe-bas (LPF) et le filtre passe-haut (HPF) ont des longueurs différentes. Le filtre passe-bas démontre la symétrie, tandis que le filtre passe-haut est asymétrique ou symétrique [19].

\subsubsection{Conclusion}
Depuis plusieurs décennies, la transformation en ondelettes s'est imposée comme un outil mathématique fondamental dans de nombreux domaines scientifiques et technologiques. Les aspects théoriques et l'analyse des ondelettes trouvent des applications dans la modélisation mathématique, les réseaux de neurones artificiels, le traitement numérique du signal, ainsi que le traitement d'images et les méthodes numériques. Le terme orthogonal relève de la partie mathématique qui couvre un large domaine du traitement numérique du signal et du traitement d'images. Une ondelette orthogonale génère une ondelette dont la nature est orthogonale. Cela signifie qu'une transformée en ondelettes inverse ou transposée n'est rien d'autre que l'adjoint d'une transformée en ondelettes. Si cette condition échoue en raison d'un manque d'orthogonalité, cela peut conduire à des ondelettes biorthogonales. Des fonctions d'échelle uniques et des ondelettes uniques sont générées, mais avec un banc de filtres d'ondelettes orthogonal. L'ondelette biorthogonale associée à la transformation en ondelettes est inversible. Si elle est inversible, elle n'a pas besoin d'être orthogonale. Dans le cas de la conception d'ondelettes orthogonales, l'ondelette biorthogonale permet une liberté maximale. Elle supporte également la construction de fonctions d'ondelettes symétriques. Dans les ondelettes biorthogonales, comme leur nom l'indique, deux facteurs ou fonctions d'échelle sont responsables de la génération de diverses multi-résolutions basées sur différentes ondelettes. Pour la reconstruction d'images et de signaux, nous avons besoin de cette ondelette. En présence d'ondelettes biorthogonales, nous obtenons de meilleurs résultats. Dans le travail actuel, nous avons analysé les performances des filtres d'ondelettes orthogonaux et biorthogonaux pour le traitement d'images. Nous testons l'image et observons les coefficients de filtre et la qualité d'image pour l'ondelette orthogonale et biorthogonale. Sur la base de l'analyse des performances, il est conclu que les ondelettes biorthogonales sont supérieures aux ondelettes orthogonales.
...
\subsection{Reconstruction d'image en Compressed Sensing par ondelettes biorthogonales 4.4 et ADMM}
...
% \subsubsection{Réduction du nombre de mesures.}\text{}\\ 
...
% \subsubsection{Problèmes inverses mal posés (ill-posed inverse problems).}\text{}\\
...
% \subsubsection{Robustesse au bruit et aux données incomplètes.} \text{}\\
...
% \subsubsection{Imagerie médicale.}
% \begin{itemize}
...
% \subsubsection{Imagerie computationnelle.}
% \begin{itemize}

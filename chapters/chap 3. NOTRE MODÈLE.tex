...
de données incomplètes et bruitées, rendant la reconstruction d'image plus difficile.\\
...
Le \emph{Compressive Sensing} (CS) fournit un cadre théorique et algorithmique permettant
d'aborder cette problématique en exploitant des propriétés structurelles des images CT.

\begin{definition}
    Le \emph{Compressive Sensing} (CS) est un cadre mathématique et algorithmique permettant la
...
Compressive Sensing à la reconstruction tomographique.
...
moins égal à la dimension du signal, le Compressive Sensing montre que
\begin{equation}
    m \gtrsim k \log(n/k)
\end{equation}
...
\subsection{Régularisation par parcimonie et Compressive Sensing}

Pour rendre le problème inverse traitable, il est nécessaire d'introduire des informations
a priori sur la solution recherchée. Le Compressive Sensing propose d'utiliser la parcimonie
...
de projections.\\
Jusqu'à présent, le Compressive Sensing a été présenté comme un cadre
...
\begin{definition}
Un \emph{problème inverse} consiste à déterminer une quantité inconnue $\mathbf{x}$ à partir d'observations indirectes $\mathbf{y}$, reliées par un opérateur $\mathcal{A}$, lorsque l'inversion directe de cet opérateur est soit impossible, soit numériquement instable. L'objectif est de reconstruire ou d'estimer $\mathbf{x}$ en exploitant les informations contenues dans $\mathbf{y}$, souvent en introduisant des contraintes ou des connaissances a priori pour régulariser la solution.
\end{definition}
...
\begin{itemize}[label=-]
    ...
    lissée (grand $\alpha$).
\end{itemize}
Le Compressive Sensing s'inscrit naturellement dans ce cadre variationnel en
...
variation totale. 
Ces choix conduisent à des problèmes d'optimisation
...
\end{equation}
La minimisation de la variation totale correspond à la minimisation de la norme $\ell_1$ du gradient et constitue une pénalisation standard dans le cadre du Compressive Sensing appliqué au CT.

\begin{figure}[H]
    \centering
    \begin{subfigure}{0.4\textwidth}
        \centering
        \includegraphics[height=4cm, keepaspectratio]{./images/shepp-logan phantom.png}
    ...
    \begin{subfigure}{0.4\textwidth}
        \centering
        \includegraphics[height=4cm, keepaspectratio]{./images/shepp-logan phantom gradient.png}
        \caption{Le gradient}
...
\begin{proposition}[Reconstruction CT par Compressive Sensing]
...
\begin{equation}L(x, y) = f(x) + y^T(Ax - b)\end{equation}
et la fonction duale est
\begin{equation}g(y) = \inf_x L(x, y) = -f^*(-A^Ty) - b^Ty,\end{equation}
où \( y \) est la variable duale ou le multiplicateur de Lagrange, et \( f^* \) est le conjugué convexe de \( f \).\\
Le problème duale est
\begin{equation}\text{maximiser } g(y),\end{equation}
...
\begin{equation}x^* = \arg\min_x L(x, y^*),\end{equation}
...
Étape 1: 
\begin{equation}x^{k+1} := \arg\min_x L(x, y^k) \end{equation}
Étape 2: 
\begin{equation}y^{k+1} := y^k + \alpha^k (Ax^{k+1} - b),\end{equation}
...
\begin{equation}f(x) = \sum_{i=1}^N f_i(x_i),\end{equation}
...
\begin{equation}A = [A_1 \cdots A_N],\end{equation}

ainsi \( Ax = \sum_{i=1}^N A_i x_i \), le Lagrangien peut s'écrire comme

\begin{equation}L(x, y) = \sum_{i=1}^N L_i(x_i, y) = \sum_{i=1}^N \left( f_i(x_i) + y^T A_i x_i - (1/N)y^T b \right),\end{equation}
...
Ce problème est clairement équivalent au problème original \eqref{eq:optimisation_convexe}, puisque pour tout \( x \) réalisable, le terme ajouté à l'objectif est nul. La fonction duale associée est \begin{equation}g_\rho(y) = \inf_x L_\rho(x, y)\end{equation}.
...
\begin{equation}Ax^* - b = 0, \quad \nabla f(x^*) + A^T y^k = 0,\end{equation}
...
\subsection{Étapes sur l'algorithme Alternating Direction Method of Multipliers}
...
\begin{equation}p^\star = \inf \{ f(x) + g(z) | Ax + Bz = c \}.\end{equation}

Comme dans la méthode des multiplicateurs, nous formons le Lagrangien augmenté

\begin{equation}L_\rho(x, z, y) = f(x) + g(z) + y^T(Ax + Bz - c) + (\rho/2) \|Ax + Bz - c\|_2^2.\end{equation}
...
\begin{equation}u^k = u^0 + \sum_{j=1}^k r^j,\end{equation}
la somme cumulée des résidus.\\
...
\begin{figure}[H]
    \centering
    \includegraphics[height=4cm, keepaspectratio]{./images/orthogonality.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[height=6cm , keepaspectratio]{./images/wt.png}
...
\begin{equation}\psi_{a,T}(t) = \frac{1}{\sqrt{a}}\psi \left(\frac{t - T}{a}\right) \end{equation}
La transformée en ondelettes $W(a, T)$ est représentée par :
\begin{equation}
    W_{x}(a,T) = \int_{-\infty}^{\infty}y(t)\frac{1}{\sqrt{a}}\Psi^{n}\left(\frac{t - T}{a}\right)dt 
    \label{eq:wt_func}
\end{equation}
...
\begin{equation}
    \left|H_{\theta}(\omega)\right|^{2} + \left|H_{\theta}(\omega +\pi)\right|^{2} = 2
...
\end{equation}
Le filtre qui satisfait la relation ci-dessus est appelé filtre miroir conjugué \cite{19}. Un filtre miroir conjugué est l'un des moyens les plus simples pour la construction d'une base d'ondelettes orthogonale. Le filtre miroir conjugué et la multi-résolution sont les méthodes de base pour la base d'ondelettes orthogonale \cite{20}\\
...
\begin{equation}
    ...
    \end{array} 
\end{equation}
À partir de l'équation mathématique ci-dessus, nous obtenons le nouveau signal 
\begin{equation}\mathrm{s} = [\mathrm{s}_1\mathrm{D}]\end{equation}
\enlargethispage{\baselineskip}
...
Le calcul consiste à faire la différence entre chaque pixel de l'image originale et de l'image reconstruite, à élever cette différence au carré afin d'éviter les compensations entre erreurs positives et négatives et de pénaliser davantage les grandes erreurs, puis à en faire la moyenne sur l'ensemble de l'image.
Une valeur de MSE égale à zéro signifie que les deux images sont identiques, ce qui correspond à une reconstruction parfaite. À l'inverse, plus la valeur de la MSE est élevée, plus la différence entre l'image originale et l'image reconstruite est importante, ce qui indique une perte de qualité significative de la reconstruction.
...
\subsection{Résolution du problème inverse par régularisation parcimonieuse}
...
\subsubsection{Formulation variationnelle du problème de reconstruction en tomodensitométrie régularisée}
...
\subsubsection{Séparation de variables et algorithme Alternating Direction Method of Multipliers}
...
\subsubsection*{Mise à jour de \(x\) (sous-problème quadratique)}
...
\subsubsection*{Mise à jour de \(z\) (seuillage doux)}
...
\subsubsection*{Mise à jour de la variable duale \(u\)}
...
\section*{Conclusion}
Ce chapitre établit les fondements d'un modèle de reconstruction pour la tomodensitométrie à faible dose, où la réduction des mesures transforme l'opération en un problème inverse mal posé. Pour stabiliser ce processus, l'approche adoptée mobilise le Compressive Sensing en exploitant la parcimonie des images dans une base d'ondelettes biorthogonales bior4.4, reconnue pour sa capacité à préserver les structures géométriques via une régularisation par norme $\ell_1$. La résolution de ce problème variationnel repose sur l'algorithme ADMM (Alternating Direction Method of Multipliers), qui découple efficacement la fidélité aux données de la régularisation par une succession d'étapes d'optimisation convexe (mise à jour quadratique et seuillage doux). Complété par un protocole d'évaluation rigoureux s'appuyant sur les métriques MSE, PSNR et SSIM, ce cadre algorithmique cohérent assure la robustesse et la fidélité structurelle des reconstructions, posant ainsi les bases nécessaires à la validation expérimentale en conditions cliniques réelles.